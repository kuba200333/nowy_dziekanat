<?php
// Plik: index.php (Wersja z poprawionym linkiem do obecności)
session_start();
...
// Kontrola dostępu (ACL) - Zaktualizowana lista dozwolonych stron
$dozwolone_strony = [];
if ($user_role == 'pracownik') {
    if (!$is_admin) {
        $dozwolone_strony = [
            'dashboard', 'oceny_czastkowe_wybor', 'oceny_czastkowe_zarzadzaj',
            'oceny_wybor_krok1', 'oceny_wybor_krok2', 'oceny_wprowadz_form', 'plan_wyszukiwarka',
            'obecnosc_wybor_zajec', 'obecnosc_form' // POPRAWIONA LISTA
        ];
    }
                ...
                <ul>
                    <?php if ($user_role == 'pracownik'): ?>
                        <?php if ($is_admin): // ### MENU DLA ADMINISTRATORA ### ?>
                            ...
                            <li><a href="index.php?page=plan_wyszukiwarka" class="<?= str_starts_with($currentPage, 'plan_wyszukiwarka') ? 'active' : '' ?>">Wyszukiwarka Planu</a></li>
                            <li><a href="index.php?page=plan_dodaj_szablon" class="<?= str_starts_with($currentPage, 'plan_dodaj_szablon') ? 'active' : '' ?>">Zarządzaj Szablonami</a></li>
                            <li><a href="index.php?page=plan_modyfikuj_termin" class="<?= str_starts_with($currentPage, 'plan_modyfikuj_termin') ? 'active' : '' ?>">Modyfikuj Terminy</a></li>
                             <hr>
