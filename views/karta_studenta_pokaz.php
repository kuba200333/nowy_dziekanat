...
// Znajdź zakres semestrów dla studenta, aby stworzyć nawigację
...
// --- NOWA CZĘŚĆ - OBLICZANIE STATYSTYK ---
...
// Statystyki globalne (dla całego toku studiów)
...
foreach($wszystkie_zapisy_res as $zapis) {
    ...
    if(isset($oceny_calkowite[$zapis['konfiguracja_id']])) {
        ...
        if((is_numeric($ocena_calk) && $ocena_calk >= 3.0) || $ocena_calk == 'zal') {
            $ects_zdobyte_total += $zapis['punkty_ects'];
        }
    }
}

// Określenie semestrów dla bieżącego roku akademickiego
$semestry_w_roku = [];
if ($biezacy_semestr % 2 != 0) { // Semestr nieparzysty (zimowy)
    $semestry_w_roku = [$biezacy_semestr, $biezacy_semestr + 1];
} else { // Semestr parzysty (letni)
    $semestry_w_roku = [$biezacy_semestr - 1, $biezacy_semestr];
}

// Średnie ważone i ECTS dla semestru/roku
$oceny_do_srednich_sql = "
    SELECT oc.wartosc_obliczona, kp.punkty_ects, g.semestr
    FROM OcenyCalkowiteZPrzedmiotu oc
    JOIN KonfiguracjaPrzedmiotu kp ON oc.konfiguracja_id = kp.konfiguracja_id
    JOIN Zajecia z ON z.konfiguracja_id = oc.konfiguracja_id
    JOIN GrupyZajeciowe g ON z.grupa_id = g.grupa_id
    WHERE oc.numer_albumu = ?
    GROUP BY oc.ocena_calkowita_id
";
...
foreach($oceny_do_srednich_res as $ocena) {
    ...
    // ECTS zdobyte w semestrze
    if ($ocena['semestr'] == $biezacy_semestr && ((is_numeric($ocena['wartosc_obliczona']) && $ocena['wartosc_obliczona'] >= 3.0) || $ocena['wartosc_obliczona'] == 'zal')) {
...
// Średnie z komponentów
$oceny_komponentow_sql = "
    SELECT z.forma_zajec, ok.wartosc_oceny FROM OcenyKoncoweZKomponentu ok
    JOIN ZapisyStudentow zs ON ok.zapis_id = zs.zapis_id
    JOIN Zajecia z ON zs.zajecia_id = z.zajecia_id
    JOIN GrupyZajeciowe g ON z.grupa_id = g.grupa_id
    WHERE zs.numer_albumu = ? AND ok.czy_zatwierdzona = TRUE AND g.semestr = ?
";
...
foreach($oceny_komp_avg_res as $ocena) {
    if(is_numeric($ocena['wartosc_oceny'])) {
        ...
        if(!isset($sumy_ocen_komp[$forma])) {
            $sumy_ocen_komp[$forma] = 0;
            $liczniki_ocen_komp[$forma] = 0;
        }
...
foreach($formy as $forma) {
    if(isset($sumy_ocen_komp[$forma])) {
        $srednie_komponenty[$forma] = round($sumy_ocen_komp[$forma] / $liczniki_ocen_komp[$forma], 2);
    } else {
        $srednie_komponenty[$forma] = "Brak danych";
    }
}
?>
...
<div style="display: flex; justify-content: space-between; align-items: center; background: #333; color: white; padding: 10px; border-radius: 5px;">
    <a href="?page=karta_studenta_pokaz&numer_albumu=<?= $numer_albumu ?>&semestr=<?= $biezacy_semestr - 1 ?>" class="btn-add" <?= ($biezacy_semestr <= $min_semestr) ? 'style="visibility: hidden;"' : '' ?>>
        &laquo; Poprzedni semestr
    </a>
    <h2 style="margin: 0; color: white;">Semestr: <?= $biezacy_semestr ?></h2>
    <a href="?page=karta_studenta_pokaz&numer_albumu=<?= $numer_albumu ?>&semestr=<?= $biezacy_semestr + 1 ?>" class="btn-add" <?= ($biezacy_semestr >= $max_semestr) ? 'style="visibility: hidden;"' : '' ?>>
